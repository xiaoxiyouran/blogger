var scriptargs = document.getElementById('testscript').getAttribute('callBack');
// alert(scriptargs)
			...
			if (this.rendered) {
				if (p_parentNode==undefined) {
					...
					if (p_parentNode.childNodes.length==0) {
						...
						v_img.src = scriptargs + '/packages/doc_tree/jquery-tree/images/collapse.png';
						...
						v_img.id = 'toggle_off';
							...
							v_img = p_parentNode.elementLi.getElementsByTagName("img")[0];
							v_img.style.visibility = "visible";
							v_img.src = scriptargs + '/packages/doc_tree/jquery-tree/images/expand.png';
							v_img.id = 'toggle_on';
						}
					}
      ...
      this.adjustLines(document.getElementById(this.name),true);
		...
		drawNode: function(p_ulElement,p_node) {
			...
			if (p_node.childNodes.length == 0) {
				v_exp_col = createImgElement('toggle_off','exp_col',scriptargs + '/packages/doc_tree/jquery-tree/images/collapse.png');
				...
				if (p_node.expanded) {
					...
					v_exp_col = createImgElement('toggle_on','exp_col',scriptargs + '/packages/doc_tree/jquery-tree/images/expand.png');
				}
			}
		...
		expandNode: function(p_node) {
			if (p_node.childNodes.length>0 && p_node.expanded==false) {
				...
				img.id="toggle_off";
				img.src = scriptargs + '/packages/doc_tree/jquery-tree/images/collapse.png';
		...
		collapseNode: function(p_node) {
			if (p_node.childNodes.length>0 && p_node.expanded==true) {
				...
				img.id="toggle_on";
				img.src = scriptargs + '/packages/doc_tree/jquery-tree/images/expand.png';
				elem_ul = img.parentElement.getElementsByTagName("ul")[0];
				...
				if (p_node.contextMenu!=undefined) {
					...
					for (var i=0; i<v_menu.elements.length; i++) (function(i){
						...
						if (v_menu.elements[i].submenu!=undefined) {
							...
							v_span_more.appendChild(createImgElement(null,'menu_img', scriptargs + '/packages/doc_tree/jquery-tree/images/right.png'));
